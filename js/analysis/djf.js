...
// This module exposes the DJF numeric API that plot rendering calls to clean
...
// aggregate/doublet masking used before fitting. It statically imports the
// Levenberg-Marquardt solver and the ml-gsd peak finder; because this module is
// itself loaded on demand (see plotting/djf_loader.js), that whole numeric stack
// stays off the initial load path. It intentionally does not draw anything; the
// plotting modules turn these model outputs into curves, components, and readouts.
...
function detect_peaks(points, threshold = null) {
  ...
  try {
    ...
    const found = gsd({ x: xs, y: ys }, { minMaxRatio: min_max_ratio, smoothY: true, realTopDetection: true });
    const peaks = (found || [])
...
export function prepare_row(row, corrections) {
...
export function components(value, p) {
...
export function estimate_run_g1(series, threshold = null) {
  const positions = [];
...
export function fit(points, range, threshold, g1_hint) {
...
export function phase_stats(points, p) {
  const stats = {
...
export function fractions(points, p) {
...
export function correction_summary(prepared_rows, corrections) {
...
Purpose:
	Finds the optional height/width companion channels for a selected DNA-area
	channel, given a raw FCS summary. Thin adapter over the shared implementation
	in fcs/channel_cleaning.js so external callers (unit tests) can keep passing a
	summary rather than a pre-built parameter map. The height/width matching logic
	itself is not duplicated here.

Input:
	summary [Object]:         parsed FCS header/metadata for one file
	selected_label [string]:  the chosen DNA-content area channel label
