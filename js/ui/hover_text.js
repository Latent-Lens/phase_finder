...
const HoverText = Object.freeze({
  ...
  cellCycleModelSelect: `Choose which cell-cycle model to fit — you always pick it directly; there is no automatic selector. Dean–Jett and Dean–Jett–Fox are generative two/three-component fits (Fox adds a broadened S-phase wave). Watson Classic is the generative model commercial tools like ModFit implement. Watson Pragmatic is a local peak-plus-residual decomposition, never compared against the generative models by AIC/BIC. CLOCCS (Unverified) jointly fits a strain's timepoints together, using the timepoint/strain metadata columns you map below once selected — "Unverified" is a deliberate trust caveat on this model, not a stale one.`,
  ...
  checkDomainSensitivity: `Re-fit this sample against several nearby analysis domains and bin counts, and check whether the reported phase fractions move more than expected. This is an on-demand check (roughly 12 extra fits) — it does not run automatically, so use it before an export or a final review rather than after every region tweak.`,
  checkResamplingUncertainty: `Re-fit this sample (and any model it's compared against by AIC/BIC) over many perturbed variants — bootstrapped events, jittered peak regions, and neighboring bin/domain choices — and report an interval for each phase fraction from the spread of the answers, plus how often model selection would have picked a different model. This is a deliberately expensive, on-demand check (up to a couple hundred extra fits per model) — it does not run automatically, so use it before an export or a final review rather than after every region tweak.`,
});
